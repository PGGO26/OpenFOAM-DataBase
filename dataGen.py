...
freestream_angle  = 0.  # -angle ... angle
freestream_length = 12.           # len * (1. ... factor)
# freestream_length_factor = 6.    # length factor
freestream_length_factor = 3

# Mutiprocessing using
cpu_to_use = mp.cpu_count()
print('This is cpu core: ',cpu_to_use)
    ...
    for x in range(res):
        for y in range(res):
            ...
            if abs(p_U[curIndex1][0] - xf)<1e-5 and abs(p_U[curIndex1][1] - yf)<1e-5:
                ...
                mapOutput1[4][x][y] = p_U[curIndex1][3]
                mapOutput1[5][x][y] = p_U[curIndex1][4]
                curIndex1 += 1 
            else:
                mapOutput1[2][x][y] = 1.0

    for x2 in range(len(point)):
        mapInput2[0][x2] = point[curIndex2][1]
...
def full_process(airfoil, fsX, fsY, n):
    ...
    if newPoint() != 0:
        ...
        os.chdir('..')
        print('\tnewPoint is done\n')
        return(-1)

    print('doing gemMesh')

    # if genMesh("../" + airfoil_database + f'{basename}.dweat') != 0:
    if genMesh("../" + airfoil_database + airfoil) != 0:
        print("\tmesh generation failed, aborting")
        os.chdir("..")
        return -1
    print('genMesh is done.')
    
    runSim(fsX, fsY)
    print('runSim is done.')
...
if __name__=='__main__':
    ...
    for n in range(samples):
        ...
        pool.apply_async(full_process, args=(airfoil, fsX, fsY, n), callback=log_id)
        #r.wait()
