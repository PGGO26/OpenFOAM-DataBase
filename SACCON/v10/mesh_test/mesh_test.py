# boundaryProbes 中的 mode 選擇 All
# 後續 0 資料夾中不能有 wallshearstress 資料夾
# reconstructPar 最終輸出的資料夾，跟 controlDict 設定的會不一致，後續會影響 postProcessing
...
for mach in MACH:
    for aoa in AOA:
        ...
        upper_outputs = boundaryProbes(xmin=X_MIN_MAX[0], xmax=X_MIN_MAX[1], ymin=Y_MIN_MAX[0], ymax=Y_MIN_MAX[1], zmin=Zu_MIN_MAX[0], zmax=Zu_MIN_MAX[1], res=RES, mode='All')

        print("Post-processing boundaryProbes lower surface :", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())))
        lower_outputs = boundaryProbes(xmin=X_MIN_MAX[0], xmax=X_MIN_MAX[1], ymin=Y_MIN_MAX[0], ymax=Y_MIN_MAX[1], zmin=Zl_MIN_MAX[0], zmax=Zl_MIN_MAX[1], res=RES, mode='All')
