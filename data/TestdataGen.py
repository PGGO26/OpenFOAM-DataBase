...
ProjectUsing_samples   = 21         # no. of da,tasets to produce
samples = 21
cpu_to_use = 5
freestream_angle  = np.pi / 18.  # -angle ... angle
length = 50
...
output_dir        = f"./test/{airfoil}/v{length}/"
...
def outputProcessing(basename, stepfile, freestreamX, freestreamY, cpu_id, dataDir=output_dir):
    point = np.load(f'{cpu_id}/data0.npy')
    shear = pd.read_csv(f'{cpu_id}/temps/shear')

    if round(freestreamY, 3) == 0.000:
        fsAOA = 0
    else:
        fsAOA = ((np.arctan(freestreamY / freestreamX)) / np.pi) * 180

    print(f'fsAOA : {fsAOA}')
    mapInput = np.zeros((1, 103, 1))
    mapOutput = np.zeros((1, 101, 1))
    curIndex1 = 0
    curIndex2 = 0

    for x1 in range(len(point)):
        mapInput[0][x1] = point[curIndex1][1]
        mapInput[0][-2] = freestreamX
        mapInput[0][-1] = freestreamY
        curIndex1 += 1

    for x2 in range(len(shear)):
        mapOutput[0][x2] = shear['shear stress'].iloc[curIndex2]
        curIndex2 += 1

    fileName = dataDir + f'{basename}' + f' {round(fsAOA, 3)}'

    print("\tsaving in " + fileName + ".npz")
    np.savez_compressed(fileName, input = mapInput, cf=mapOutput)

utils.makeDirs( [output_dir, "./OpenFOAM/constant/polyMesh/sets", "./OpenFOAM/constant/polyMesh"] )
...
if __name__=='__main__':
    ...
    for n in range(samples):
        ...
        angle = (-freestream_angle) + ((freestream_angle / ((ProjectUsing_samples - 1) / 2)) * n)
        fsX =  np.cos(angle) * length
        fsY = np.sin(angle) * length
    ...
    print(f'Final time elapsed: {np.floor(totalTime / 60)} hr {np.floor(totalTime % 60)} minutes')
    with open('log', 'a+') as infile:
        infile.write(f'(total  running samples : {len(samplelst)}) / cpu core : {cpu_to_use}) total time : {np.floor(totalTime / 60)} hr {np.floor(totalTime % 60)} min\n')